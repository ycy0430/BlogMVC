...
namespace web.Areas.admin.Controllers
{
    ...
    public class rbac_RoleController : baseController
    {
        ...
        //展示关联岗位
        public ActionResult LinkJobs(string rowID)
        {
            return View();
        }
        ...
        public JsonResult Priv(string rowID)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    List<string> list = rui.requestHelper.getList("detail.resourceCode");
                    foreach (string keyCode in list)
                    {
                        //获取授权的操作
                        {
                            string value = rui.requestHelper.getMulValue(keyCode + ".cbx");
                            dicOP.Add(keyCode, rui.typeHelper.toString(value));
                        }
                        //获取授权的数据权限(下拉框）
                        {
                            string value = rui.requestHelper.getValue(keyCode + ".ddl");
                            string dataPriv = rui.requestHelper.getValue(keyCode + ".dataPriv");
                            string haveDataPriv = rui.requestHelper.getValue(keyCode + ".haveDataPriv");
