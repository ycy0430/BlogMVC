...
namespace db.bll
{
    ...
    public class rbac_User
    {
        ...
        public static void update(db.rbac_User entry, db.dbEntities dc)
        {
            ...
            orgEntry.isAllPrj = entry.isAllPrj;

            //如果职工的组织等于登录组织，则同步职工的部门
            {
                db.sbs_Empl emplEntry = dc.sbs_Empl.Single(a => a.emplCode == entry.userCode);
                if (emplEntry.orgCode == orgEntry.orgCode)
            ...
            efHelper.entryUpdate(entry, dc, "deptCode,isLogin,isAllPrj");
            dc.SaveChanges();
        }
        ...
        public static db.rbac_User getEntryByRowID(string rowID, db.dbEntities dc)
        {
            ...
            if (orgEntry != null)
            {
                ...
                entry.isAllPrj = orgEntry.isAllPrj;
            }
            return entry;
        }
        ...
        /// <param name="dc"></param>
        public static void createRelatedUser(string userCode, string userName, string orgCode, string deptCode, db.dbEntities dc)
        {
            efHelper ef = new efHelper(ref dc);
