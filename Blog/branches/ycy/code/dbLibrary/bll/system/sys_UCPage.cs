...
namespace db.bll
{
    ...
    public class sys_UcPage
    {
        ...
        public static void update(string resourceCode, int cPageSize, int cPageWidth, db.dbEntities dc)
        {
            ...
            List<string> fieldCodeList = rui.requestHelper.getList("detail.fieldCode");
            List<string> CFieldNameList = rui.requestHelper.getList("detail.CFieldName");
            List<string> showOrderList = rui.requestHelper.getList("detail.showOrder");
            List<string> colWidthList = rui.requestHelper.getList("detail.colWidth");
            List<string> fixedValueList = rui.requestHelper.getList("detail.fixedValue");
            List<string> alignTypeList = rui.requestHelper.getList("detail.alignType");
            ...
            for (int i = 0; i < fieldCodeList.Count; i++)
            {
                ...
                entry.isShow = rui.requestHelper.getValue(fieldCode + ".isShow", false) == "" ? false : true;
                entry.showOrder = rui.typeHelper.toInt(showOrderList[i]);
                entry.isOrder  = rui.requestHelper.getValue(fieldCode + ".isOrder", false) == "" ? false : true;
                entry.colWidth = rui.typeHelper.toIntAllowNull(colWidthList[i]);
                entry.isResize = rui.requestHelper.getValue(fieldCode + ".isResize", false) == "" ? false : true;
