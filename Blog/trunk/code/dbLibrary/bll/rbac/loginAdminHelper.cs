...
namespace db.bll
{
    ...
    public class loginAdminHelper
    {
        ...
        public static void login(string userCode, string password, string orgCode, dbEntities dc)
        {
            ...
            {
                string sql = @" SELECT userName,rbac_UserOrg.deptCode,deptName,orgName,isAllPrj FROM rbac_User
                            LEFT JOIN rbac_UserOrg
                            ON rbac_User.userCode = rbac_UserOrg.userCode
                             AND orgCode = @orgCode
                            LEFT JOIN sbs_Dept ON rbac_UserOrg.deptCode = sbs_Dept.deptCode
                            LEFT JOIN sbs_Org ON sbs_Dept.orgCode = sbs_Org.orgCode
                            WHERE isLogin = '是'
                             AND rbac_User.userCode=@userCode AND password=@password ";
                ...
                if (row != null)
                {
                    ...
                    rui.sessionHelper.saveValue<string>("admin.orgName", row["orgName"].ToString());
                    rui.sessionHelper.saveValue<string>("admin.deptCode", row["deptCode"].ToString());
                    rui.sessionHelper.saveValue<string>("admin.deptName", row["deptName"].ToString());
                    rui.sessionHelper.saveValue<string>("admin.isAllPrj", row["isAllPrj"].ToString());
                    //保存当前登录用户拥有的权限
        ...
        {
            if (rui.configHelper.getAppValue("isDev") == "true")
                return true;
